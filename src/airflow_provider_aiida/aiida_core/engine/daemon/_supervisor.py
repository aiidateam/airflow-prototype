...
# We need auto-register pattern because we need to be able to load the the specialized ServiceConfigs from the json file, so we potentially need to be able to create a ServiceConfig just from the service_name
...
class WorkerServiceConfig(ServiceConfig):
    ...
    # TODO need to rethink since for airflow it makes more sense to 
    #      start the sync workers all at once
    @abstractmethod
    def create_unique_env(self) -> dict[str, str]:
        raise NotImplementedError()
...
class ServiceSupervisorController:
    ...
    def status(supervisor_dir: Path) -> dict:
        """
        Get status of all services by reading supervisor config and service info files.

        Reads the supervisor config file to discover all configured services,
        then reads each service's info file to return current status information.

        Returns:
            dict: Status dictionary with structure:
                {
                    'session': str,
                    'supervisor': {
                        'pid': int,
        ...
        if not config_file.exists():
            status_dict['error'] = f"No supervisor config file found at {config_file}"
            return status_dict

        try:
            service_configs = ServiceConfigMap.from_file(config_file)
        except Exception as e:
            status_dict['error'] = f"Error reading supervisor config: {e}"
            return status_dict
        ...
        if supervisor_info_file.exists():
            try:
                ...
                log_path = session_dir / ServiceSupervisorCommon.SUPERVISOR_LOG_FILE
                status_dict['supervisor'] = {
                    'pid': supervisor_info.pid,
                    'status': 'RUNNING' if is_alive else 'STOPPED',
                    'started': supervisor_info.create_time,
                    'log': str(log_path)
                }
            except Exception as e:
                status_dict['supervisor'] = {'error': f"Error reading info - {e}"}
        else:
            status_dict['supervisor'] = {'error': "No info file found"}
        ...
        for service_identifier, config in service_configs.items():
            if isinstance(config, NonWorkerServiceConfig):
                ...
                if info_file.exists():
                    ...
                    service_entry['error'] = "No info file found"

                status_dict['services'][config.service_name] = service_entry
                ...
                for worker_num in range(config.num_workers):
                    ...
                    if info_file.exists():
                        ...
                        worker_entry['error'] = "No info file found"

                    service_entry['workers'][worker_num] = worker_entry
    ...
    @staticmethod
    def get_service_configs(supervisor_dir: Path) -> ServiceConfigMap | None:
        session_dir = ServiceSupervisorController._get_latest_session_dir(supervisor_dir)
        if session_dir is None:
            return None
        config_file = session_dir / ServiceSupervisorCommon.SUPERVISOR_CONFIG_FILE
        try:
            return ServiceConfigMap.from_file(config_file)
        except Exception as e:
            # TODO addd full traceback
            logger.warning(f"Could not read supervisor config file {config_file}, due to exception: {e}")
