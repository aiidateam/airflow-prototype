...
async def task_upload_job(node: CalcJobNode, transport_queue: TransportQueue, cancellable: InterruptableFuture, calc_info=None):
    ...
    :param transport_queue: the TransportQueue from which to request a Transport (unused, replaced with get_transport_queue)
    :param cancellable: the cancelled flag that will be queried to determine whether the task was cancelled
    :param calc_info: optional CalcInfo from prepare_for_submission
    ...
    async def do_upload():
        ...
        # AIRFLOW-AIIDA-MODIFICATION: Get transport queue from provider package instead of parameter
        # This ensures we use the current event loop instead of a stale one
        t_get_queue_start = time.time()
        from airflow_provider_aiida.aiida_core.transport import get_transport_queue
        transport_queue = get_transport_queue()
        t_get_queue = time.time() - t_get_queue_start
        logger.info(f"CalcJob<{node.pk}> upload: get_transport_queue took {t_get_queue*1000:.1f}ms")

        t_request_start = time.time()
        with transport_queue.request_transport(authinfo) as request:
            t_request_setup = time.time() - t_request_start
            logger.info(f"CalcJob<{node.pk}> upload: request_transport setup took {t_request_setup*1000:.1f}ms")

            t_transport_await_start = time.time()
            transport = await cancellable.with_interrupt(request)
            t_transport_await = time.time() - t_transport_await_start
            logger.info(f"CalcJob<{node.pk}> upload: waiting for transport took {t_transport_await*1000:.1f}ms")

            t_sandbox_start = time.time()
            with SandboxFolder(filepath_sandbox) as folder:
                t_sandbox_create = time.time() - t_sandbox_start
                logger.info(f"CalcJob<{node.pk}> upload: SandboxFolder creation took {t_sandbox_create*1000:.1f}ms")

                # Restore folder contents from node's repository
                # (they were stored by prepare_for_submission task)
                t_copy_start = time.time()
                node.base.repository.copy_tree(folder.abspath, path=None)
                t_copy = time.time() - t_copy_start
                logger.info(f"CalcJob<{node.pk}> upload: repository.copy_tree took {t_copy*1000:.1f}ms")

                # Use the provided calc_info or create a minimal one
                if calc_info is None:
                    from aiida.common.datastructures import CalcInfo
                    calc_info_to_use = CalcInfo()
                    calc_info_to_use.uuid = str(node.uuid)
                    calc_info_to_use.skip_submit = False
                else:
                    calc_info_to_use = calc_info

                t_upload_start = time.time()

                # Detailed timing wrapper for upload_calculation
                logger.info(f"CalcJob<{node.pk}> upload: Starting execmanager.upload_calculation...")

                # Monkey-patch transport methods to add timing
                original_whoami = transport.whoami_async if hasattr(transport, 'whoami_async') else None
                original_path_exists = transport.path_exists_async if hasattr(transport, 'path_exists_async') else None
                original_makedirs = transport.makedirs_async if hasattr(transport, 'makedirs_async') else None
                original_mkdir = transport.mkdir_async if hasattr(transport, 'mkdir_async') else None
                original_put = transport.put_async if hasattr(transport, 'put_async') else None

                # Stats tracking
                transport_stats = {
                    'whoami_calls': 0, 'whoami_time': 0,
                    'path_exists_calls': 0, 'path_exists_time': 0,
                    'makedirs_calls': 0, 'makedirs_time': 0,
                    'mkdir_calls': 0, 'mkdir_time': 0,
                    'put_calls': 0, 'put_time': 0,
                }

                async def timed_whoami(*args, **kwargs):
                    t = time.time()
                    result = await original_whoami(*args, **kwargs)
                    transport_stats['whoami_time'] += time.time() - t
                    transport_stats['whoami_calls'] += 1
                    return result

                async def timed_path_exists(*args, **kwargs):
                    t = time.time()
                    result = await original_path_exists(*args, **kwargs)
                    transport_stats['path_exists_time'] += time.time() - t
                    transport_stats['path_exists_calls'] += 1
                    return result

                async def timed_makedirs(*args, **kwargs):
                    t = time.time()
                    result = await original_makedirs(*args, **kwargs)
                    transport_stats['makedirs_time'] += time.time() - t
                    transport_stats['makedirs_calls'] += 1
                    return result

                async def timed_mkdir(*args, **kwargs):
                    t = time.time()
                    result = await original_mkdir(*args, **kwargs)
                    transport_stats['mkdir_time'] += time.time() - t
                    transport_stats['mkdir_calls'] += 1
                    return result

                async def timed_put(*args, **kwargs):
                    t = time.time()
                    result = await original_put(*args, **kwargs)
                    transport_stats['put_time'] += time.time() - t
                    transport_stats['put_calls'] += 1
                    return result

                # Apply patches
                if original_whoami:
                    transport.whoami_async = timed_whoami
                if original_path_exists:
                    transport.path_exists_async = timed_path_exists
                if original_makedirs:
                    transport.makedirs_async = timed_makedirs
                if original_mkdir:
                    transport.mkdir_async = timed_mkdir
                if original_put:
                    transport.put_async = timed_put

                try:
                    remote_folder = await execmanager.upload_calculation(node, transport, calc_info_to_use, folder)
                finally:
                    # Restore original methods
                    if original_whoami:
                        transport.whoami_async = original_whoami
                    if original_path_exists:
                        transport.path_exists_async = original_path_exists
                    if original_makedirs:
                        transport.makedirs_async = original_makedirs
                    if original_mkdir:
                        transport.mkdir_async = original_mkdir
                    if original_put:
                        transport.put_async = original_put

                t_upload = time.time() - t_upload_start

                # Log detailed breakdown
                logger.info(
                    f"CalcJob<{node.pk}> upload: execmanager.upload_calculation took {t_upload*1000:.1f}ms ({t_upload:.3f}s)\n"
                    f"  Transport operation breakdown:\n"
                    f"    whoami:      {transport_stats['whoami_calls']:2d} calls, {transport_stats['whoami_time']*1000:7.1f}ms total\n"
                    f"    path_exists: {transport_stats['path_exists_calls']:2d} calls, {transport_stats['path_exists_time']*1000:7.1f}ms total\n"
                    f"    makedirs:    {transport_stats['makedirs_calls']:2d} calls, {transport_stats['makedirs_time']*1000:7.1f}ms total\n"
                    f"    mkdir:       {transport_stats['mkdir_calls']:2d} calls, {transport_stats['mkdir_time']*1000:7.1f}ms total\n"
                    f"    put:         {transport_stats['put_calls']:2d} calls, {transport_stats['put_time']*1000:7.1f}ms total\n"
                    f"  Unaccounted time: {(t_upload - sum([transport_stats['whoami_time'], transport_stats['path_exists_time'], transport_stats['makedirs_time'], transport_stats['mkdir_time'], transport_stats['put_time']]))*1000:.1f}ms"
                )

                # AIRFLOW-AIIDA-MODIFICATION: Removed process.out('remote_folder', remote_folder)
                # Original AiiDA code stores remote_folder as an output link via process.out()
                # which is allowed on stored nodes. We tried to store it as an attribute instead,
                # but attributes cannot be modified on stored+unsealed nodes (only _updatable_attributes can).
                # Since we don't have process context to create output links, we skip storing remote_folder.
                # The remote working directory path is already tracked in the node's internal state.
                skip_submit = calc_info_to_use.skip_submit or False

            t_total = time.time() - t_start
            logger.info(
                f"CalcJob<{node.pk}> upload: do_upload total time {t_total*1000:.1f}ms ({t_total:.3f}s) - "
                f"breakdown: queue={t_get_queue*1000:.1f}ms, transport_wait={t_transport_await*1000:.1f}ms, "
                f"copy={t_copy*1000:.1f}ms, upload={t_upload*1000:.1f}ms"
            )

            return skip_submit

    try:
        logger.info(f'scheduled request to upload CalcJob<{node.pk}> (initial_interval={initial_interval}s, max_attempts={max_attempts})')
        ...
        t_retry_total = time.time() - t_retry_start
        logger.info(f'CalcJob<{node.pk}> upload: exponential_backoff_retry completed in {t_retry_total*1000:.1f}ms ({t_retry_total:.3f}s)')
        ...
        t_set_state = time.time() - t_set_state_start
        logger.info(f'CalcJob<{node.pk}> upload: set_state took {t_set_state*1000:.1f}ms')
...
def submit_calculation(calculation: CalcJobNode, transport: Transport) -> str | ExitCode:
    ...
    if job_id is not None:
        logger.debug(f"CalcJob<{calculation.pk}> submit_calculation: job already has ID {job_id}, skipping")
        return job_id

    t_scheduler_start = time.time()
    scheduler = calculation.computer.get_scheduler()
    scheduler.set_transport(transport)
    t_scheduler_setup = time.time() - t_scheduler_start

    submit_script_filename = calculation.get_option('submit_script_filename')
    workdir = calculation.get_remote_workdir()

    t_submit_job_start = time.time()
    result = scheduler.submit_job(workdir, submit_script_filename)
    t_submit_job = time.time() - t_submit_job_start

    t_set_job_id_start = time.time()
    if isinstance(result, str):
        calculation.set_job_id(result)
    t_set_job_id = time.time() - t_set_job_id_start

    logger.info(
        f"CalcJob<{calculation.pk}> submit_calculation: "
        f"scheduler_setup={t_scheduler_setup*1000:.1f}ms, "
        f"submit_job={t_submit_job*1000:.1f}ms, "
        f"set_job_id={t_set_job_id*1000:.1f}ms"
    )
...
async def task_submit_job(node: CalcJobNode, transport_queue: TransportQueue, cancellable: InterruptableFuture):
    ...
    async def do_submit():
        ...
        with transport_queue.request_transport(authinfo) as request:
            ...
            #result = execmanager.submit_calculation(node, transport)
            result = submit_calculation(node, transport)
            t_submit = time.time() - t_submit_start

            t_total = time.time() - t_start
            logger.info(
                f"CalcJob<{node.pk}> submit: do_submit total {t_total*1000:.1f}ms - "
                f"transport_wait={t_transport_await*1000:.1f}ms, submit={t_submit*1000:.1f}ms"
            )

            return result

    try:
        logger.info(f'scheduled request to submit CalcJob<{node.pk}> (initial_interval={initial_interval}s, max_attempts={max_attempts})')
        ...
        t_retry_total = time.time() - t_retry_start
        logger.info(f'CalcJob<{node.pk}> submit: exponential_backoff_retry completed in {t_retry_total*1000:.1f}ms ({t_retry_total:.3f}s)')
        ...
        t_set_state = time.time() - t_set_state_start
        logger.info(f'CalcJob<{node.pk}> submit: set_state took {t_set_state*1000:.1f}ms')
...
async def task_update_job(node: CalcJobNode, job_manager, cancellable: InterruptableFuture):
    ...
    async def do_update():
        ...
        with job_manager.request_job_info_update(authinfo, job_id) as update_request:
            ...
            t_job_info_await_start = time.time()
            job_info = await cancellable.with_interrupt(update_request)
            t_job_info_await = time.time() - t_job_info_await_start
        ...
        t_state_update = time.time() - t_state_update_start

        t_total = time.time() - t_start
        logger.info(
            f"CalcJob<{node.pk}> update: do_update total {t_total*1000:.1f}ms - "
            f"job_info_wait={t_job_info_await*1000:.1f}ms, state_update={t_state_update*1000:.1f}ms, done={job_done}"
        )

        return job_done

    try:
        logger.info(f'scheduled request to update CalcJob<{node.pk}> (initial_interval={initial_interval}s, max_attempts={max_attempts})')
        ignore_exceptions = (plumpy.futures.CancelledError, plumpy.process_states.Interruption)
        ...
        t_retry_total = time.time() - t_retry_start
        logger.info(f'CalcJob<{node.pk}> update: exponential_backoff_retry completed in {t_retry_total*1000:.1f}ms ({t_retry_total:.3f}s)')
        ...
        if job_done:
            ...
            t_set_state = time.time() - t_set_state_start
            logger.info(f'CalcJob<{node.pk}> update: set_state took {t_set_state*1000:.1f}ms')
...
async def task_retrieve_job(
    ...
    async def do_retrieve():
        ...
        with transport_queue.request_transport(authinfo) as request:
            transport = await cancellable.with_interrupt(request)
            t_transport = time.time() - t_transport_start
            ...
            if node.get_job_id() is None:
                ...
                t_job_info = time.time() - t_job_info_start

                t_retrieve_start = time.time()
                retrieved = await execmanager.retrieve_calculation(node, transport, retrieved_temporary_folder)
                t_retrieve = time.time() - t_retrieve_start
                logger.info(f'CalcJob<{node.pk}> retrieve: job_info={t_job_info*1000:.1f}ms')

            # AIRFLOW-AIIDA-MODIFICATION: Manually create output link instead of process.out()
            # Original AiiDA code: process.out(node.link_label_retrieved, retrieved)
            # Since we don't have process context, we manually create the output link
            t_link_start = time.time()
            if retrieved is not None:
                retrieved.base.links.add_incoming(node, link_type=LinkType.CREATE, link_label=node.link_label_retrieved)
                retrieved.store()
            t_link = time.time() - t_link_start

            t_total = time.time() - t_start
            logger.info(
                f"CalcJob<{node.pk}> retrieve: do_retrieve total {t_total*1000:.1f}ms ({t_total:.3f}s) - "
                f"transport={t_transport*1000:.1f}ms, retrieve={t_retrieve*1000:.1f}ms, link={t_link*1000:.1f}ms"
            )

            return retrieved

    try:
        logger.info(f'scheduled request to retrieve CalcJob<{node.pk}> (initial_interval={initial_interval}s, max_attempts={max_attempts})')
        ignore_exceptions = (plumpy.futures.CancelledError, plumpy.process_states.Interruption)
        ...
        t_retry_total = time.time() - t_retry_start
        logger.info(f'CalcJob<{node.pk}> retrieve: exponential_backoff_retry completed in {t_retry_total*1000:.1f}ms ({t_retry_total:.3f}s)')
        ...
        t_set_state = time.time() - t_set_state_start
        logger.info(f'CalcJob<{node.pk}> retrieve: set_state took {t_set_state*1000:.1f}ms')
