...
def validate_script(ctx, param, value: str):
    ...
    if not os.path.isfile(value):
        raise ValueError(f'The script file: {value} does not exist')
    if not os.access(value, os.X_OK):
        raise ValueError(f'The script {value} is not executable')
    return value


def validate_backend(ctx, param, value: str):
    if value not in ['asyncssh', 'openssh']:
        raise ValueError(f'{value} is not a valid backend, choose either `asyncssh` or `openssh`')
    return value
...
from .utils import TransportPath
...
class Transport(abc.ABC):
    ...
    def logger(self):
        ...
        try:
            ...
            raise ValueError('No self._logger configured for {}!')
...
class TransportInternalError(ValueError):
...
class AsyncSshTransport(AsyncTransport):
    ...
    async def open_async(self):
        ...
        """
        if self._is_open:
            raise ValueError('Cannot open the transport twice')
    ...
    async def close_async(self):
        ...
        if not self._is_open:
            raise ValueError('Cannot close the transport: it is already closed')
